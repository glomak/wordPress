 ...
 * @return array
 */
function yaatra_body_classes( $classes ) {
    ...
    if( 'post' === get_post_type() ) {
        $sidebar_meta_option = get_post_meta( $post->ID, 'yaatra_sidebar_layout', true );
    }
    ...
    if( is_home() ) {
        ...
        $sidebar_meta_option = get_post_meta( $home_id, 'yaatra_sidebar_layout', true );
    }
    
    if( empty( $sidebar_meta_option ) || is_archive() || is_search() ) {
        $sidebar_meta_option = 'default_sidebar';
    }
    $archive_sidebar        = get_theme_mod( 'yaatra_archive_sidebar', 'right_sidebar' );
    $post_default_sidebar   = get_theme_mod( 'yaatra_global_post_sidebar', 'right_sidebar' );
    $page_default_sidebar   = get_theme_mod( 'yaatra_global_page_sidebar', 'right_sidebar' );
    ...
    // Archive layout
    $yaatra_archive_layout = get_theme_mod( 'yaatra_archive_layout', 'grid' );
    if( is_archive() || is_home() || is_search() ) {
        $classes[] = $yaatra_archive_layout.'-archive-layout';
    }

    return $classes;
}
add_filter( 'body_class', 'yaatra_body_classes' );

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function yaatra_pingback_header() {
    if ( is_singular() && pings_open() ) {
        echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
    }
}
add_action( 'wp_head', 'yaatra_pingback_header' );

/*----------------------------------------------------------------------------------------------------------------------------------------*/
/**
 * Register Google fonts for yaatra
 *
 * @return string Google fonts URL for the theme.
 * @since 1.0.0
 */
if ( ! function_exists( 'yaatra_fonts_url' ) ) :
    function yaatra_fonts_url() {
        $fonts_url = '';
        $font_families = array();
         ...
         */
        if ( 'off' !== _x( 'on', 'Playfair Display font: on or off', 'yaatra' ) ) {
            $font_families[] = 'Playfair Display:400,700';
        }

        if ( 'off' !== _x( 'on', 'Overpass font: on or off', 'yaatra' ) ) {
            $font_families[] = 'Overpass:300,400,600,700';
        }

        if ( 'off' !== _x( 'on', 'Pacifico font: on or off', 'yaatra' ) ) {
 ...
 * @since 1.0.0
 */
function yaatra_admin_scripts_style( $hook ) {
    ...
    wp_enqueue_style( 'yaatra-admin-styles', get_template_directory_uri() .'/assets/css/cv-admin-styles.css', array(), esc_attr( $yaatra_theme_version ) );
    wp_enqueue_script( 'jquery-ui-button' );
    wp_enqueue_script( 'yaatra-admin-scripts', get_template_directory_uri() .'/assets/js/cv-admin-scripts.js', array('jquery'), esc_attr( $yaatra_theme_version ), true );
    
}
add_action( 'admin_enqueue_scripts', 'yaatra_admin_scripts_style' );

/*----------------------------------------------------------------------------------------------------------------------------------------*/
/**
 * Enqueue scripts and styles.
 */
function yaatra_scripts() {
    global $yaatra_theme_version;

    wp_enqueue_style( 'yaatra-fonts', yaatra_fonts_url(), array(), null );
    ...
    wp_enqueue_style( 'yaatra-style', get_stylesheet_uri(), array(), esc_attr( $yaatra_theme_version ) );
    wp_enqueue_style( 'yaatra-responsive-style', get_template_directory_uri() . '/assets/css/cv-responsive.css', array(), esc_attr( $yaatra_theme_version ) );

    wp_enqueue_script( 'yaatra-navigation', get_template_directory_uri() . '/assets/js/navigation.js', array(), '20151215', true );
    wp_enqueue_script( 'yaatra-skip-link-focus-fix', get_template_directory_uri() . '/assets/js/skip-link-focus-fix.js', array(), '20151215', true );
    ...
    wp_enqueue_script( 'yaatra-custom-scripts', get_template_directory_uri() . '/assets/js/cv-custom-scripts.js', array('jquery'), esc_attr( $yaatra_theme_version ), true );

    $yaatra_wow_option = get_theme_mod( 'yaatra_wow_option', true );
    if( $yaatra_wow_option === true ) {
        $wow_value = 'on';
    } else {
        $wow_value = 'off';
    }

    wp_localize_script( 'yaatra-custom-scripts', 'yaatraObject', array(
...
add_action( 'wp_enqueue_scripts', 'yaatra_scripts' );

/*----------------------------------------------------------------------------------------------------------------------------------------*/

if( ! function_exists( 'yaatra_font_awesome_icon_array' ) ) :
    /**
     ...
     * @since 1.0.0
     */
    function yaatra_font_awesome_icon_array(){
...
if( ! function_exists( 'yaatra_font_awesome_social_icon_array' ) ) :
    /**
     ...
     * @since 1.0.0
     */
    function yaatra_font_awesome_social_icon_array(){
        return array(
 ...
 * @since 1.0.0
 */
if( ! function_exists( 'yaatra_get_sidebar' ) ):
function yaatra_get_sidebar() {
    ...
    if( is_home() ) {
        ...
        $sidebar_meta_option = get_post_meta( $set_id, 'yaatra_sidebar_layout', true );
    }
...
/*----------------------------------------------------------------------------------------------------------------------------------------*/
if ( ! function_exists( 'yaatra_inner_header_bg_image' ) ) :
    /**
     * Background image for inner page header
     *
     * @since 1.0.0
     */
    function yaatra_inner_header_bg_image( $input ) {
        $image_attr = array();
        if ( empty( $image_attr ) ) {
...
add_filter( 'yaatra_inner_header_style_attribute', 'yaatra_inner_header_bg_image' );
 ...
 * @since 1.0.0
 */
if( !function_exists( 'yaatra_social_media' ) ):
    function yaatra_social_media() {
...
/*----------------------------------------------------------------------------------------------------------------------------------------*/
/**
 * Yaatra Dynamic styles
 */
add_action( 'wp_enqueue_scripts', 'yaatra_dynamic_styles' );
if( ! function_exists( 'yaatra_dynamic_styles' ) ) :
    function yaatra_dynamic_styles() {
        $yaatra_theme_color = get_theme_mod( 'yaatra_theme_color', '#27B6D4' );

        $output_css = '';
        $output_css .= ".reply .comment-reply-link,.widget_search .search-submit,.widget_search .search-submit,article.sticky:before,.widget_search .search-submit:hover, .widget-title::before, .widget-title::after, #site-navigation ul li a:hover, .site-header.widget_categories li a:hover, #site-navigation ul li a:hover{ background: ". esc_attr( $yaatra_theme_color ) ."}\n";
        $output_css .= "a,a:hover,a:focus,a:active,.entry-footer a:hover,.comment-author .fn .url:hover,.commentmetadata .comment-edit-link,#cancel-comment-reply-link,#cancel-comment-reply-link:before,.logged-in-as a,.widget a:hover,.widget a:hover::before,.widget li:hover::before,.banner-btn a:hover,.entry-title a:hover,.entry-title a:hover,.cat-links a,.yaatra_latest_posts .cv-post-title a:hover, .entry-btn a:hover, h1.entry-title span, .cv-social-icons-wrapper a:hover, #footer-menu li a:hover, .menu-toggle:hover,.widget_tag_cloud a:hover, #cv-scrollup:hover,.search-no-results #primary .search-submit:hover,.banner-btn a:hover i{ color: ". esc_attr( $yaatra_theme_color ) ."}\n";
        $output_css .= ".widget_search .search-submit, .widget_search .search-submit:hover, .entry-btn a:hover, .widget_tag_cloud a:hover,.site-header .widget_categories li a:hover{ border-color: ". esc_attr( $yaatra_theme_color ) ."}\n";

        $refine_output_css = yaatra_css_strip_whitespace( $output_css );
        wp_add_inline_style( 'yaatra-style', $refine_output_css );
    }
endif;
 ...
 * @since 1.0.0
 */
if( ! function_exists( 'yaatra_css_strip_whitespace' ) ) :
    function yaatra_css_strip_whitespace( $css ){
        $replace = array(
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
if( ! function_exists( 'yaatra_related_posts_section' ) ) :
    /**
     * function to display the related posts section at single post page.
     */
    function yaatra_related_posts_section() {
        $related_posts_option = get_theme_mod( 'yaatra_enable_related_posts', true );
        if ( true === $related_posts_option && is_single() ) {
            get_template_part( 'template-parts/related-posts/related', 'posts' );
        }
    }
endif;
add_action( 'yaatra_after_single_page', 'yaatra_related_posts_section', 10 );

if( !function_exists( 'yaatra_read_more_button' ) ):
    /**
    * function to use filter for read more button label
    *
    * @since 1.0.0
    */
    function yaatra_read_more_button() {
        $button_label = apply_filters( 'yaatra_read_more_button_label', __( 'Continue Reading', 'yaatra' ) );

         echo '<div class="entry-btn"><a href="'. esc_url( get_the_permalink() ) .'">'.esc_html( $button_label ).'</a></div>';
    }
endif;

/*-----------------------------------------------------------------------------------------------------------------------*/

add_filter( 'wp_kses_allowed_html', 'yaatra_required_data_attributes' , 10, 4 );

if( ! function_exists( 'yaatra_required_data_attributes' ) ) :
     ...
     * @since 1.0.5
     */
    function yaatra_required_data_attributes( $required_attributes, $context ) {
